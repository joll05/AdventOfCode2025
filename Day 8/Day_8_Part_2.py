...
def connect_all(boxes: list[JunctionBox]):
    while True:
        lowest_distance = boxes[0].get_closest()[0]
        box_to_connect = boxes[0]
        ...
        first, second = box_to_connect.connect_closest()
        if box_to_connect.circuit.size() == len(boxes):
            print(f"Final connection: {first} and {second}")
            print(f"X Coordinate Product: {first.x * second.x}")
            return
